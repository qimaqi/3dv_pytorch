...
class UNet_Nested(nn.Module):
    ...
    def __init__(self, n_channels=256, n_classes=1, feature_scale=1, is_deconv=False, is_batchnorm=True, is_ds=True):
    ...
    def forward(self, inputs):
        # column : 0
        X_00 = self.conv00(inputs)       # 16*512*512    qi:256*256*256
        maxpool0 = self.maxpool(X_00)    # 16*256*256       qi:256*128*128
        X_10= self.conv10(maxpool0)      # 32*256*256    qi:256*128*128
        maxpool1 = self.maxpool(X_10)    # 32*128*128       qi:256*64*64
        X_20 = self.conv20(maxpool1)     # 64*128*128    qi:512*64*64
        maxpool2 = self.maxpool(X_20)    # 64*64*64         qi:512*32*32
        X_30 = self.conv30(maxpool2)     # 128*64*64     qi:512*32*32
