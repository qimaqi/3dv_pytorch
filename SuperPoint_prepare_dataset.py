...
if __name__ == '__main__':
    ## what you should do
    base_image_dir='/cluster/scratch/ruiwang46/invsfm'

    save_dir='/cluster/scratch/ruiwang46/invsfm/data/superpoint_5k'
    cuda = False
    ##
    ...
    for i in range(len(image_list)):
        ...
        if len(pts[:,2])>10000:
            new_pts=pts[:,0:10000]
            new_desc=desc[:,0:10000]
            np.savez_compressed(os.path.join(save_dir,save_name), pts=new_pts, desc=new_desc)
        else:
            np.savez_compressed(os.path.join(save_dir,save_name), pts=pts, desc=desc)
