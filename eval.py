...
def eval_net(net, loader, device):
    ...
    for batch in loader:
        ...
        # debug part
        tmp_output_dir = '/cluster/scratch/jiaqiu/debug_output_eval_invnet/' +str(global_step) + '.png'
        tmp_img_dir = '/cluster/scratch/jiaqiu/debug_images_eval_invnet/'+ str(global_step) + '.png'
...
def visib_eval_net(net, loader, device):
    """Evaluation without the densecrf with the dice coefficient"""
    net.eval()
    mask_type = torch.float32 if net.n_classes == 1 else torch.long
    n_val = len(loader)  # the number of batch
