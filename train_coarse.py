...
from utils.dataset import BasicDataset4
...
dir_checkpoint = '/cluster/scratch/ruiwang46/checkpoints/'
...
base_image_dir='/cluster/scratch/ruiwang46/npz_torch_data'
...
for i in range(len(train_5k_image_rgb)):
    ...
    feature_list.append(os.path.join(base_image_dir,'superpoint_5k',superpoint_feature_name))
    ...
    # temp=np.load(os.path.join(base_image_dir,'superpoint_5k',superpoint_feature_name))
    # pts=temp['pts']
    # desc=temp['desc']
    # temp=np.load(os.path.join(base_image_dir,train_5k_image_depth[i]))['image_depth']
...
def train_net(net,
    ...
    dataset = BasicDataset4(image_list, depth_list, feature_list,img_scale, pct_3D_points, crop_size)
