...
class InferDataset(Dataset):
    def __init__(self, image_list, feature_list, max_points):
        self.image_list = image_list
        self.feature_list = feature_list
        self.max_points = max_points
        self.ids = list(range(len(feature_list)))
        logging.info('Creating dataset with %s examples', len(self.ids))
    ...
    def __getitem__(self, i):
        ...
        img = Image.open(self.image_list[idx]).convert('RGB') # read img in greyscale
        temp=np.load(self.feature_list[idx],allow_pickle=True)
        pos=temp['pts']
        desc=temp['desc']
        ...
        img = np.array(img)
        height, width, _ = np.shape(img)  # (480,640) with scale
        ...
        feature = feature_pad.transpose((2, 0, 1)) # channel x 480 x 640
...
class BasicDatasetR2D2(Dataset):
    def __init__(self, image_list, feature_list, max_points, crop_size, scale):
        self.image_list = image_list
        self.feature_list = feature_list
        self.max_points = max_points
        self.crop_size = crop_size
    ...
    @classmethod
    def preprocess(cls, feature, img, crop_size, scale):
        # feature: HWC, img in np shape: HWC. img in size WHC
        h, w, c = np.shape(feature) 
        feature_nd = np.array(feature)
        ...
        # HWC to CHW 
        feature_trans = feature_rnd.transpose((2, 0, 1)) 
    ...
    def __getitem__(self, i):
        ...
        # after preprocess, the feature and image will be well transposed and augumented
        feature, img = self.preprocess(feature_pad, img, self.crop_size, self.scale)   ### QM: the process only transpose channel, need more data augumentation
